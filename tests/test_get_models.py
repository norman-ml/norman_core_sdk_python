...
from tests.test_utils import api_client_logged_in


@pytest.mark.asyncio
async def test_get_models(api_client_logged_in):
    api_client, login_response = api_client_logged_in
    response = await Models.get_models(api_client, login_response.access_token)
    assert len(response) == 0 or isinstance(response[0], Model)

@pytest.mark.asyncio
async def test_get_account_models(api_client_logged_in):
    api_client, login_response = api_client_logged_in
    ...
    response = await Models.get_models(api_client, login_response.access_token, get_models_request)
