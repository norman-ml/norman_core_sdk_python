...
class Retrieve(metaclass=Singleton):
    ...
    async def get_model_asset(self, token: Sensitive[str], account_id: str, version_id: str, asset_id: str) -> Any:
    ...
    async def get_invocation_input(self, token: Sensitive[str], account_id: str, version_id: str, invocation_id: str, input_id: str) -> Any:
        endpoint = f"retrieve/input/{account_id}/{version_id}/{invocation_id}/{input_id}"
        return await self._http_client.get(endpoint, token, response_encoding=ResponseEncoding.Iterator)

    async def get_invocation_output(self, token: Sensitive[str], account_id: str, version_id: str, invocation_id: str, output_id: str) -> Any:
