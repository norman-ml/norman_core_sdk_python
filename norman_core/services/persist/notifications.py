...
class Notifications:
    @staticmethod
    async def get_notifications(api_client: ApiClient, token: Sensitive[str], constraints: Optional[QueryConstraints] = None):
        json = None
        if constraints is not None:
            json = constraints.model_dump(mode="json")

        response = await api_client.post("persist/notifications/get", token, json=json)
